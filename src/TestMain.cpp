...
int main(int argc, char** argv){
	...
	for (auto i = gvlist.begin(), e = gvlist.end(); i != e; i++)
	{
		...
		vector<struct InstInfo *> DirectDataFlow = gv_info->getDirectDataFlow();
		llvm::outs() <<"getDirectDataFlow size= "<<DirectDataFlow.size()<<"\n";
		vector<struct InstInfo *> IndirectDataFlow = gv_info->getIndirectDataFlow();
		llvm::outs() <<"getIndirectDataFlow size= "<<IndirectDataFlow.size()<<"\n";
	}
	return 0;
}
