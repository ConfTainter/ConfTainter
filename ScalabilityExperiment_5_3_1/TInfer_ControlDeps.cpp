...
int main(int argc, char** argv){
	...
	startAnalysis(gvlist, false, false);		//Taint Analysis with no indirect data-flow, and no indirect control-flow
	...
	for (auto i = gvlist.begin(), e = gvlist.end(); i != e; i++){
		for (auto j = gvlist.begin(); j != e; j++) if(i!=j){
			...
			vector<struct InstInfo*> info_list1 = gv_info1->getDirectDataFlow();
			vector<struct InstInfo*> info_list2 = gv_info2->getDirectDataFlow();
			int flag = 0;
